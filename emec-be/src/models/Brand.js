...
class Brand {
  static async findAll(page = 1, limit = 10) {
    ...
    const [rows] = await pool.execute(
      `SELECT * FROM vehicle_brands WHERE is_deleted = 0 ORDER BY created_at DESC LIMIT ${offset}, ${limitNum}`
    );
    const [count] = await pool.execute(
      `SELECT COUNT(*) as total FROM vehicle_brands WHERE is_deleted = 0`
    );
